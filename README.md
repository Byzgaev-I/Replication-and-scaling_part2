...
- backend определяет тип данных и ключ шардинга;  
- запрос на запись направляется в master нужного шарда;  
- запрос на чтение направляется в slave;  
- репликация выполняется от master к slave.

### 5. В итоге получаем следующую логику системы

- вертикальный шардинг разделяет данные по смыслу;  
- горизонтальный шардинг распределяет нагрузку внутри сущностей;  
- master/slave репликация обеспечивает отказоустойчивость и масштабирование чтения;  
- архитектура вполне подходит для высоконагруженных систем.
